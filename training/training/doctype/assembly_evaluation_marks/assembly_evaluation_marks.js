...
frappe.ui.form.on('Assembly Evaluation Marks', {
	...
	refresh: function(frm) {
		...
		frm.add_custom_button(__('Send Mail to Supervisor'), function () {
			...
			frm.save()
			frappe.call({
				method: "training.training.doctype.assembly_evaluation_marks.assembly_evaluation_marks.sendmail",
				args: {
					"name": frm.doc.name,
	...
	get_employees(frm){
		if((frm.doc.evaluation_marks).length==0){
			frappe.call({
				method: "training.training.doctype.assembly_evaluation_marks.assembly_evaluation_marks.get_employees",
				args: {
	...
	submit(frm,cdt,cdn){
		...
		$.each(frm.doc.evaluation_marks,function(i,d){
			...
			check_list = [d.cable_bunching_sub_assy,d.laying,d.tapping,d.ckt_packing,d.parts_fitting]
			if(list.includes(0)){
		...
		frappe.call({
			method: "training.training.doctype.assembly_evaluation_marks.assembly_evaluation_marks.submit",
			args: {
...
frappe.ui.form.on('Observation Marks AE', {
	open(frm,cdt,cdn){
		var child = locals[cdt][cdn]
