...
frappe.ui.form.on('Auto Cutting and Crimping Marks', {
	...
	refresh: function(frm) {
		...
		frm.add_custom_button(__('Send Mail to Supervisor'), function () {
			...
			frm.save()
			frappe.call({
				method: "training.training.doctype.auto_cutting_and_crimping_marks.auto_cutting_and_crimping_marks.sendmail",
				args: {
					"name": frm.doc.name,
	...
	get_employees(frm){
		...
		frappe.call({
			method: "training.training.doctype.auto_cutting_and_crimping_marks.auto_cutting_and_crimping_marks.get_employees",
			args: {
				"from_date": frm.doc.evaluation_date,
	...
	submit(frm,cdt,cdn){
		...
		frappe.call({
			method: "training.training.doctype.auto_cutting_and_crimping_marks.auto_cutting_and_crimping_marks.submit",
			args: {
...
frappe.ui.form.on('Observation Marks', {
open(frm,cdt,cdn){
	var child = locals[cdt][cdn]
	frappe.set_route("Form","Auto Cutting and Crimping",child.id)
},
