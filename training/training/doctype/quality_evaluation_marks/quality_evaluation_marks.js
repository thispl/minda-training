...
frappe.ui.form.on('Quality Evaluation Marks', {
	...
	refresh: function(frm) {
		...
		frm.add_custom_button(__('Send Mail to Supervisor'), function () {
			...
			frm.save()
			frappe.call({
				method: "training.training.doctype.quality_evaluation_marks.quality_evaluation_marks.sendmail",
				args: {
					"name": frm.doc.name,
	...
	get_employees(frm){
		...
		frappe.call({
			method: "training.training.doctype.quality_evaluation_marks.quality_evaluation_marks.get_employees",
			args: {
	...
	submit(frm,cdt,cdn){
		...
		$.each(frm.doc.evaluation_marks,function(i,d){
			...
			check_list = [d.visual_inspector,d.patrol_inspector,d.pdi]
			if(list.includes(0)){
		...
		frappe.call({
			method: "training.training.doctype.quality_evaluation_marks.quality_evaluation_marks.submit",
			args: {
...
frappe.ui.form.on('Observation Marks QE', {
	open(frm,cdt,cdn){
		var child = locals[cdt][cdn]
