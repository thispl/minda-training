...
def details(emp_id):
    if frappe.db.exists("Induction Training Assembly Area",emp_id):
        return "NA"
    elif frappe.db.exists("Induction Training Machine Area Crimping",emp_id):
        doc = frappe.get_doc("Induction Training Machine Area Crimping",emp_id)
        if doc.is_machine_area_operator_applicable == 1:
            if frappe.db.exists("Induction Training Machine Area Operator",emp_id):
                return True
            else: 
                return False
        else:
